 ...
 * @link       http://www.clearfoundation.com/docs/developer/apps/proxy/
 */
...
use \clearos\apps\proxy\Squid as Squid;
...
clearos_load_library('proxy/Squid');
...
class Squid extends Daemon
{
    ...
    public function get_max_file_download_size()
    {
        clearos_profile(__METHOD__, __LINE__);
    ...
    public function get_max_object_size()
    {
        clearos_profile(__METHOD__, __LINE__);
    ...
    public function set_cache_size($size)
    {
        ...
        if ( (!preg_match("/^\d+/", $size)) || ($size < 0) )
            throw new Validation_Exception(SQUID_LANG_MAX_CACHE_SIZE . " - " . LOCALE_LANG_INVALID);
    ...
    public function set_max_file_download_size($size)
    {
        clearos_profile(__METHOD__, __LINE__);

        if ( (!preg_match("/^\d+/", $size)) || ($size < 0) )
            throw new Validation_Exception(SQUID_LANG_MAX_FILE_DOWNLOAD_SIZE . " - " . LOCALE_LANG_INVALID);
    ...
    public function set_max_object_size($size)
    {
        clearos_profile(__METHOD__, __LINE__);

        if ( (!preg_match("/^\d+/", $size)) || ($size < 0) )
            throw new Validation_Exception(SQUID_LANG_MAX_CACHE_OBJECT_SIZE . " - " . LOCALE_LANG_INVALID);
    ...
    protected function _size_in_bytes($size, $units)
    {
        ...
        if (! preg_match("/^\d+$/", $size))
            throw new Validation_Exception(SQUID_LANG_INVALID_SIZE);
    ...
    public function _delete_parameter($key)
    {
        clearos_profile(__METHOD__, __LINE__);
    ...
    public function _set_parameter($key, $value, $offset, $default)
    {
        if (! $this->is_loaded)
            $this->_load_config();
        ...
        try {
            ...
            if (!$match) {
                try {
                    ...
                    $file->add_linesBefore("$replacement\n", "/^#/");
                }
            }
        } catch (Exception $e) {
            throw new Engine_Exception($e->GetMessage(), COMMON_WARNING);
