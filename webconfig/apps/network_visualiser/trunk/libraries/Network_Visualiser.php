...
class Network_Visualiser
{
    ...
    function send()
    {
        ...
        if ($this->message['recipient'] == NULL || empty($this->message['recipient'])) {
            throw new Validation_Exception(lang('network_visualiser_recipient_not_set'));
        } else {
            foreach ($this->message['recipient'] as $address) {
                if ($this->validate_email($address['address']))
                    throw new Validation_Exception(lang('network_visualiser_recipient') . ' - ' . lang('base_invalid') . ' (' . $address['address'] . ')');
            }
        }
        ...
        if ($swift->send($message, $recipient_list, $sender)) {
            ...
            $swift->disconnect();
            $this->clear();
            throw new Engine_Exception(lang('network_visualiser_send_failed'), CLEAROS_WARNING);
        }
    }
    ...
    function test_relay($email)
    {
        ...
        $this->set_subject(lang('network_visualiser_test'));
        $this->set_body(lang('network_visualiser_test_success'));
        $this->send();
    }
    ...
    function get_ssl_options()
    {
        ...
        $options = array(
            \Swift_Connection_SMTP::ENC_OFF=>lang('network_visualiser_none'),
            \Swift_Connection_SMTP::ENC_SSL=>lang('network_visualiser_ssl'),
            \Swift_Connection_SMTP::ENC_TLS=>lang('network_visualiser_tls')
        );
        return $options;
    }
    ...
    public function validate_email($email)
    {
        ...
        if (!preg_match("/[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}/i", $email))
            return lang('network_visualiser_email_is_invalid');
    }
    ...
    public function validate_subject($subject)
    {
        ...
        if (preg_match("/.*\n.*/", $subject))
            return lang('network_visualiser_subject_is_invalid');
    }
    ...
    public function validate_port($port)
    {
        ...
        if ($port < 1 || $port > 65535)
            return lang('network_visualiser_port_is_invalid');
    }
    ...
    public function validate_host($host)
    {
        ...
        if ($hostname->validate_hostname($host))
            return lang('network_visualiser_host_is_invalid');
    }
    ...
    public function validate_ssl($ssl)
    {
        ...
        if (is_bool($ssl))
            return lang('network_visualiser_ssl_is_invalid');
    }
    ...
    public function validate_username($username)
    {
        ...
        if (!preg_match("/^[A-Z0-9._%+-@]*$/i", $username))
            return lang('network_visualiser_username_is_invalid');
    }
    ...
    public function validate_password($password)
    {
        ...
        if (!preg_match("/^[A-Z0-9._%+-@$\\?\\(\\)]*$/i", $password))
            return lang('network_visualiser_password_is_invalid');
    }
    ...
    function validate_attachment($attachment)
    {
        ...
        if (!is_array($attachment))
            return lang('network_visualiser_attachment_invalid');
        ...
        if (!$file->exists())
            return lang('network_visualiser_attachment_file_not_found') . ' - ' . $attachment['filename'];
    }
}
