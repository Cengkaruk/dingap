...
$anchors = array(anchor_add('/app/bandwidth/basic/add'));
...
foreach ($rules as $id => $details) {
    ...
    $item['action'] = '/app/bandwidth/basic/delete/' . $key;
    $item['anchors'] = button_set(
        array(
            $state_anchor('/app/bandwidth/basic/' . $state . '/' . $key, 'high'),
            anchor_delete('/app/bandwidth/basic/delete/' . $key . '/' . $details['service'] . '/' . $details['upstream'], 'low')
        )
    );
    $item['details'] = array(
        $details['mode_text'],
