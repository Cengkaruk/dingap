...
# ClearFoundation -- trying to track down mystery routing issue
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_VERSION: $PLUTO_VERSION"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_VERB: $PLUTO_VERB"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_CONNECTION: $PLUTO_CONNECTION"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_CONN_POLICY: $PLUTO_CONN_POLICY"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_NEXT_HOP: $PLUTO_NEXT_HOP"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_INTERFACE: $PLUTO_INTERFACE"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_ME: $PLUTO_ME"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_MY_CLIENT: $PLUTO_MY_CLIENT"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_MY_CLIENT_NET: $PLUTO_MY_CLIENT_NET"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_MY_CLIENT_MASK: $PLUTO_MY_CLIENT_MASK"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_MY_SOURCEIP: $PLUTO_MY_SOURCEIP"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_MY_PROTOCOL: $PLUTO_MY_PROTOCOL"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_MY_PORT: $PLUTO_MY_PORT"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_PEER: $PLUTO_PEER"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_PEER_CLIENT: $PLUTO_PEER_CLIENT"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_PEER_CLIENT_NET: $PLUTO_PEER_CLIENT_NET"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_PEER_CLIENT_MASK: $PLUTO_PEER_CLIENT_MASK"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_PEER_PROTOCOL: $PLUTO_PEER_PROTOCOL"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_PEER_PORT: $PLUTO_PEER_PORT"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_CONNECTION_TYPE: $PLUTO_CONNECTION_TYPE"
logger -p authpriv.notice -t ipsec "updown script parameter PLUTO_STACK: $PLUTO_STACK"
...
changesource() {
	...
	# ClearFoundation -- trying to track down mystery routing issue
	logger -p authpriv.notice -t ipsec "change source route requested: $it"
	logger -p authpriv.notice -t ipsec "route parameters: PLUTO_PEER_CLIENT = $PLUTO_PEER_CLIENT"
	logger -p authpriv.notice -t ipsec "route parameters: PLUTO_INTERFACE = $PLUTO_INTERFACE"
	logger -p authpriv.notice -t ipsec "route parameters: PLUTO_MY_SOURCEIP = $PLUTO_INTERFACE"
...
doroute() {
	...
	# ClearFoundation - TODO: weird extra route appearing. Just a temporary workaround
	CHECK=`ip route list | grep "$PLUTO_PEER_CLIENT dev"`
	if [ -n "$CHECK" ]; then
		logger -p authpriv.notice -t ipsec "skipping local route $PLUTO_PEER_CLIENT"
		return 0
	fi
	...
	*)	it="ip route $1 $parms $parms2 $parms3"
	...
	# ClearFoundation -- trying to track down mystery routing issue
	logger -p authpriv.notice -t ipsec "route requested: $it"
	logger -p authpriv.notice -t ipsec "route parameters: PLUTO_PEER_CLIENT = $PLUTO_PEER_CLIENT"
	logger -p authpriv.notice -t ipsec "route parameters: parms = $parms"
	logger -p authpriv.notice -t ipsec "route parameters: parms2 = $parms2"
	logger -p authpriv.notice -t ipsec "route parameters: parms3 = $parms3"
...
# ClearFoundation -- trying to track down mystery routing issue
if [ -x /usr/sbin/fix-vpn-routes ]; then
	logger -p authpriv.notice -t ipsec "checking routes"
