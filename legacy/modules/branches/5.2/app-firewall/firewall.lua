...
function LoadEnvironment()
	...
	if SYSWATCH_WANIF == nil then
		...
		for i, ifn in SYSWATCH_WANIF do
...
function GetUntrustedInterfaces(all_interfaces)
	...
	for _, ifn in HOTIF do
		table.insert(ifn_trusted, ifn)
	end

	for _, ifn in DMZIF do
		table.insert(ifn_trusted, ifn)
	end

	if all_interfaces == true then
		for _, ifn in WANIF_CONFIG do
			table.insert(ifn_trusted, ifn)
		end
	else
		for _, ifn in WANIF do
...
function GetTrustedInterfaces()
	...
	for _, ifn in LANIF do
		is_hot = false

		for __, ifn_hot in HOTIF do
...
function NetworkInterfaces()
	...
	for i, ifn in if_list() do
		if string.find(ifn, "^[ae]th%d$") then
			f = false
			for _, difn in difs do
				if ifn == difn then f = true end
			end

			-- PPPOEKLUDGE... sigh.
			if not f then
				for __, ifn_ppp in ifn_pppoe do
...
function RunIncomingAllowedDefaults()
	...
	for _, ifn in GetUntrustedInterfaces(true) do
		iptables("filter", "-A OUTPUT -o " .. ifn .. " -j " .. FW_ACCEPT)
...
function RunIncomingAllowed()
	...
	for _, rule in RULES do
		...
		if b_and(r_type, tonumber(os.getenv("FWR_ENABLED"))) ~= 0 and
			...
			for __, ifn in GetUntrustedInterfaces(false) do
	...
	if PPTP_SERVER == "on" then
		...
		for _, ifn in GetUntrustedInterfaces(false) do
			iptables("filter",
...
function RunIncomingDenied()
	...
	if PROTOCOL_FILTERING == "on" then
		...
		for protocol, mark in config do
...
function RunProxyPorts()
	...
	elseif SQUID_TRANSPARENT == "on" and bridge == false then
		...
		for _, ifn in GetTrustedInterfaces() do
			iptables("nat",
		...
		if string.len(SQUID_FILTER_PORT) ~= 0 then
			...
			for _, ifn_int in GetTrustedInterfaces() do
				iptables("nat",
					string.format("-A PREROUTING -i %s -p tcp --dport 80 -j REDIRECT --to-port %d",
...
function RunBandwidthRules()
	...
	for ifn_wan, ifn in IMQIF_UPSTREAM do
		burst = ""
	...
	for ifn_wan, ifn in IMQIF_DOWNSTREAM do
		burst = ""
...
function RunDmzPinhole()
	...
	for _, rule in RULES do
		...
		if b_and(r_type, tonumber(os.getenv("FWR_ENABLED"))) ~= 0 and
			...
			for __, ifn_lan in LANIF do
				for ___, ifn_dmz in DMZIF do
...
function RunOneToOneNat()
	...
	for _, ifn in if_list() do
		for __, ifn_wan in WANIF do
	...
	if string.lower(ONE_TO_ONE_NAT_MODE) == "type2" then
		...
		--for _, ifn_wan in WANIF do
		for _, ifn_wan in WANIF_CONFIG do
			nat_addr[ifn_wan] = {}
		end
		...
		for _, rule in RULES do
			...
			if b_and(r_type, tonumber(os.getenv("FWR_ENABLED"))) ~= 0 and
				...
				f = true
				for ___, ip in nat_addr[ifn_wan] do
					if ip == r_addr then
						f = false; break
					end
				end
		...
		for __, ifn_wan in WANIF do
			...
			for ____, ip in nat_addr[ifn_wan] do
...
function RunMultipathRouting()
	...
	for _, ifn in WANIF do
		for __, i in MULTIPATH_WEIGHTS do
...
-- Commit changes
for _, t in TABLES do iptc_commit(t) end
