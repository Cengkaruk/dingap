...
function addBlock(params) {
	alert('Add block firewall rule has not been implemented.');
	//newXMLHttpRequest('block');
...
function updateTableRows() {
	...
	for (var x= message.getElementsByTagName('entry').length -1 ; x >= 0; x--) {
		...
		if (proto == 'UDP' ||  proto == 'TCP')
			cell.innerHTML = \"<a href='JavaScript:addBlock(\\\"dstip=\" + dst + \"&dstport=\" + dstport + \"&srcip=\" + src + \"&srcport=\" + srcport + \"\\\");'>" .  WEBCONFIG_ICON_CANCEL . "</a>\";
		else
			cell.innerHTML = '';
...
function newXMLHttpRequest(block) {
	...
	if (block == undefined) {
		request.open('POST', '/admin/jnetstatus.php', true);
