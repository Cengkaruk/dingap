...
WebDialogIntro(WEB_LANG_PAGE_TITLE, "/images/icon-remote-monitor.png", WEB_LANG_PAGE_INTRO);
...
if (isset($_POST['addtocart'])) {
	try {
		$item = new ClearSdnCartItem(ClearSdnService::SDN_MONITOR);
...
function DisplayDetails()
{
	...
	echo "</div>";
	WebFormOpen($_SERVER['PHP_SELF'], "post", "remote-monitor", "id='clearsdnform' target='_blank'");
        ...
        <script language='javascript'>
          $(document).ready(function() {
            $.ajax({
              ...
              data: 'action=getServiceDetails&service=" . ClearSdnService::SDN_MONITOR . (isset($_POST['usecache']) ? "&usecache=1" : "") . "',
