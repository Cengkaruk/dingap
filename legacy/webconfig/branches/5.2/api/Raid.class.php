...
class Raid extends Engine
{
	...
	function GetPartitionTable($device)
	{
		...
		try {
			...
			throw new EngineException($e->GetMessage() . " ($device)", COMMON_WARNING);
		}
	}
	...
	function CopyPartitionTable($from, $to)
	{
		if (COMMON_DEBUG_MODE)
			self::Log(COMMON_DEBUG, "called", __METHOD__, __LINE__);
	...
	function SanityCheckPartition($array, $check)
	{
		if (COMMON_DEBUG_MODE)
			self::Log(COMMON_DEBUG, "called", __METHOD__, __LINE__);
