...
if (isset($_POST['addtocart'])) {
	try {
		$item = new ClearSdnCartItem(ClearSdnService::SDN_FILTER);
...
function DisplayDetails()
{
	...
	echo "</div>";
	WebFormOpen($_SERVER['PHP_SELF'], "post", "contentfilter", "id='clearsdnform' target='_blank'");
        ...
        <script language='javascript'>
          $(document).ready(function() {
            $.ajax({
              ...
              data: 'action=getServiceDetails&service=" . ClearSdnService::SDN_FILTER . (isset($_POST['usecache']) ? "&usecache=1" : "") . "',
